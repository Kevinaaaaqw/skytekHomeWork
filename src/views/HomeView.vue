...
<template>
    <main>
      ...
      <div v-for="movie in movie" style="border:1px solid black;display:flex;justify-content:space-between">
        <div>
        <div>{{movie.Title}}</div>
        <div>{{movie.Year}}</div>
        <div>{{movie.Type}}</div>
        ...
        <br/>
        <br/>
        <button @click='()=>{addMovieList({Title:movie.Title,Year:movie.Year,Type:movie.Type,imdbID:movie.imdbID,Poster:movie.Poster})}'>加入車車</button>
        </div>
